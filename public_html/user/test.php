 ...
 * keyingi comment satrlar HTML sifatida chiqib ketgan.
 *
 * 3 rejim:
...
if ($natija_id > 0) {
    ...
    $rang_meta = [
        'green'  => ['bg-emerald-500/15', 'text-emerald-400', 'ring-emerald-500/30', '🎉 Ajoyib natija!'],
        'blue'   => ['bg-blue-500/15',    'text-blue-400',    'ring-blue-500/30',    '👍 Yaxshi natija!'],
        'yellow' => ['bg-amber-500/15',   'text-amber-400',   'ring-amber-500/30',   '📚 Mashq davom etsin.'],
        'red'    => ['bg-red-500/15',     'text-red-400',     'ring-red-500/30',     '💪 Qayta urinib ko\'ring!'],
    ][$rang];

    $bar_cls = [
        'green' => 'bg-emerald-400', 'blue' => 'bg-blue-400',
        'yellow' => 'bg-amber-400',  'red'  => 'bg-red-400',
    ][$rang];
    ...
    <main class="max-w-3xl mx-auto px-4 py-8 pb-20">
        ...
        <div class="glass-card p-8 text-center mb-6 fade-up">
            <div class="w-24 h-24 mx-auto rounded-2xl flex items-center justify-center
                        font-display font-black text-3xl tabnum mb-5
                        <?= $rang_meta[0] ?> <?= $rang_meta[1] ?>
                        ring-4 <?= $rang_meta[2] ?>">
                <?= $foiz ?>%
            </div>
            ...
            <p class="text-sm <?= $rang_meta[1] ?> mb-4"><?= $rang_meta[3] ?></p>

            <!-- Progress bar -->
            <div class="max-w-xs mx-auto h-2.5 bg-white/[0.07] rounded-full overflow-hidden mb-5">
                <div class="h-full <?= $bar_cls ?> rounded-full transition-all duration-1000"
                     style="width:<?= $foiz ?>%"></div>
            </div>

            <!-- Statistika -->
            <div class="flex items-center justify-center gap-6 text-sm text-white/60 mb-7">
        ...
        <div class="space-y-3">
            <?php foreach ($savollar as $i => $s):
                ...
                $border  = $togri_
                    ...
                    : ($j ? 'border-red-500/30 bg-red-500/[0.02]' : '');
            ?>
            <div class="glass-card p-5 <?= $border ?> fade-up"
                ...
                <div class="flex gap-3 mb-3">
                    <span class="w-8 h-8 rounded-lg flex items-center justify-center
                                 ...
                                 <?= $togri_ ? 'bg-emerald-500/20 text-emerald-400'
                                    : ($j ? 'bg-red-500/20 text-red-400'
                                          : 'bg-white/[0.06] text-white/40') ?>">
                        <?= $i + 1 ?>
                    </span>
                    <p class="flex-1 text-sm text-white/90 leading-relaxed font-medium">
                        <?= e($s['matn']) ?>
                    </p>
                </div>
                ...
                <?php if (!empty($s['izoh'])): ?>
                <div class="mt-3 ml-11 p-3 rounded-xl bg-blue-500/10 border border-blue-500/20
                            text-xs text-blue-300 flex items-start gap-2">
...
if ($bilet_id > 0) {
    ...
    $bilet = db_qator(
        'SELECT * FROM biletlar WHERE id = ? AND holat = "faol"',
        [$bilet_id]
    );
    if (!$bilet) {
        flash_qoy('xato', t('malumot_yoq'));
    ...
    if (!$natija) {
        ...
        $savol_son  = (int) db_qiymat(
            'SELECT COUNT(*) FROM savollar WHERE bilet_id = ?', [$bilet_id]
        );
        $yid = db_bajar(
    ...
    // BUG FIX: qolgan vaqtni boshlangan dan hisoblash (qolgan_vaqt = doimiy muddat)
    $qolgan = max(0,
        (int)$natija['qolgan_vaqt'] - (time() - strtotime($natija['boshlangan']))
    );
    ...
    <div x-data="testApp(<?= htmlspecialchars(json_encode([
        ...
        <!-- ── Sticky header ──────────────────────────────── -->
        <header class="sticky top-0 z-40 bg-[#070C1A]/92 backdrop-blur-2xl border-b border-white/[0.07]">
            <div class="max-w-4xl mx-auto px-4 h-14 flex items-center gap-4">
                <div class="flex items-center gap-3 flex-1 min-w-0">
                    <a href="<?= e(SAYT_URL) ?>/dashboard"
                       class="text-white/40 hover:text-white transition p-1 flex-shrink-0">
                ...
                <div class="flex items-center gap-2 flex-shrink-0">
                    ...
                    <span class="font-mono font-bold tabnum text-sm px-3 py-1 rounded-lg transition-all"
                          :class="qolgan < 60
            ...
            <div class="h-0.5 bg-white/[0.06]">
                <div class="h-full bg-gradient-to-r from-blue-500 to-violet-500 transition-all duration-500"
            ...
            <div class="flex flex-wrap gap-1.5 mb-6">
                <template x-for="(s, i) in savollar" :key="s.id">
                    <button @click="joriy = i"
                            :class="{
                                'ring-2 ring-blue-500 bg-blue-500/25 text-white': i === joriy,
                                'bg-emerald-500/15 text-emerald-400 border border-emerald-500/20': javoblar[s.id] && i !== joriy,
                                'bg-white/[0.05] text-white/45 hover:bg-white/[0.09]': !javoblar[s.id] && i !== joriy
                            }"
                            class="w-9 h-9 rounded-lg text-xs font-bold transition-all active:scale-95 tabnum"
                            x-text="i + 1"></button>
                </template>
            </div>
                ...
                <div x-show="i === joriy"
                    ...
                    <div class="flex items-center justify-between mb-4">
                        <div class="flex items-center gap-2">
                            <span class="w-8 h-8 rounded-lg bg-blue-500/15 text-blue-400
                                         flex items-center justify-center font-bold text-xs tabnum"
                                  x-text="i + 1"></span>
                            <span class="text-xs text-white/35">/ <span x-text="savollar.length"></span></span>
                        </div>
                        <span x-show="javoblar[s.id]"
                              class="badge badge-blue font-mono uppercase text-xs"
                              x-text="javoblar[s.id]"></span>
                    </div>

                    <p class="text-base text-white/90 leading-relaxed mb-5 font-medium"
                       x-text="s.matn"></p>

                    <template x-if="s.rasm">
                        <img :src="'<?= e(SAYT_URL) ?>/uploads/' + s.rasm"
                             class="rounded-2xl mb-5 max-w-sm w-full mx-auto block"
                             loading="lazy" alt="">
                    </template>
                        ...
                        <template x-for="v in ['a','b','c','d']" :key="v">
                            <button x-show="s['variant_' + v]"
                                    ...
                                    :class="javoblar[s.id] === v
                                        ? 'border-blue-500 bg-blue-500/18 text-white shadow-lg shadow-blue-500/15'
                                        : 'border-white/[0.09] bg-white/[0.03] text-white/75 hover:bg-white/[0.07] hover:border-white/20'"
                                    class="w-full text-left p-4 rounded-xl border-[1.5px]
                                           transition-all duration-150 flex items-center gap-3 group
                                           active:scale-[0.99]">
                                <span class="w-7 h-7 rounded-lg flex items-center justify-center
                                             text-xs font-bold uppercase flex-shrink-0 transition-colors"
                                      :class="javoblar[s.id] === v
                                          ? 'bg-blue-500 text-white'
                                          : 'bg-white/[0.07] text-white/45 group-hover:text-white/70'"
                                      x-text="v"></span>
                    ...
                    <div class="mt-3 h-4 text-xs text-center transition-all"
                         :class="{
                             'text-emerald-400': saqlash === 'saqlandi',
                             'text-red-400':     saqlash === 'xato',
                             'text-white/30':    saqlash === 'saqlanmoqda'
                         }">
        ...
        <div x-show="modal" x-transition
            ...
            <div class="glass-card p-7 max-w-sm w-full scale-in" @click.stop>
                ...
                <h3 class="text-xl font-display font-black text-center mb-2">
                    <?= e(t('tasdiqlaysizmi')) ?>
                </h3>
                <div class="text-sm text-center text-white/55 space-y-1 mb-6">
                    <p>Javob berilgan:
                        <strong class="text-white" x-text="Object.keys(javoblar).length"></strong>
                ...
                <div class="flex gap-3">
                    <button @click="modal = false" class="btn btn-ghost flex-1">
                        <?= e(t('bekor_qilish')) ?>
                    </button>
                    <button @click="testTugat(false)"
                            class="btn btn-success flex-1" :disabled="loading">
        ...
        return {
            ...
            init() {
                ...
                try {
                    ...
                    if (saved) {
                        this.javoblar = Object.assign({}, JSON.parse(saved), this.javoblar);
                    }
                } catch(e) {}
            ...
            async testTugat(vaqtTugadi) {
                ...
                if (r && r.ok) {
                    ...
                    alert(r ? r.xato : 'Xato yuz berdi. Sahifani yangilang.');
                    this.modal = true;
                }
            },
...
<main class="max-w-7xl mx-auto px-4 py-8 pb-20">
    ...
    <div class="grid sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        ...
        <div class="glass-card <?= $kirish ? 'glass-card-hover' : 'opacity-60' ?> p-5 fade-up relative flex flex-col">
            ...
            <p class="font-semibold text-sm text-white/85 mb-1 pr-2 leading-tight">
                <?= e($b['nomi']) ?>
            </p>
            ...
            <a href="<?= e(SAYT_URL) ?>/test?bilet=<?= (int)$b['id'] ?>"
               class="btn btn-primary w-full btn-sm mt-auto">
                <?= e(t('test_boshlash')) ?>
            </a>
            <?php else: ?>
            <a href="<?= e(SAYT_URL) ?>/tolov"
               class="btn btn-ghost w-full btn-sm mt-auto opacity-60">
                🔒 Tarif olish
            </a>
            <?php endif; ?>
        </div>
